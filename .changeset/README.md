...
3. The version workflow (`.github/workflows/version.yml`) opens or updates a "Version Packages" PR that bumps `package.json` and `CHANGELOG.md`. If a feature PR already drafted `## <next version>` with bullets, that section is kept; changesets still consume and bump the package.
