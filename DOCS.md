...
Package-level research decisions are recorded in `DECISIONS.md`. Default pack skills live in `skills/<id>/SKILL.md`. `frontend-design` is a playbook with `setup` / `build` / `review` / `detect` modes; detect YAML matches audit-only asks and fails slogan-hero + ticket-stack first viewports; the 2026-09-12 GitHub scan is in `research/summaries/frontend-design-agent-skill-2026-09-12.md`. `accessibility-wcag` is a WCAG 2.1 AA playbook (keyboard in the running browser); scan notes are in `research/summaries/accessibility-wcag-skill-2026-09-12.md`. `testing-qa` is unit / regression / smoke with commands listed; it does not replace `browser-qa` or `supabase-auth-rls`. `browser-qa` names unexpected console errors and failed same-origin requests. `ship` is go / no-go with named env, rollback, kill switch, and `browser-qa` paths for UI. `nextjs-app-router` requires one Action/Handler error shape and official docs or UNVERIFIED. Optional `debug` (`agent-kit add skill debug`) is reproduce → localize → reduce → fix → guard; not on default `init`. Optional `docs` updates USER_GUIDE, CHANGELOG, and the living file that moved; Reject restoring the 17-doc OS. Optional `upgrade` is `agent-kit update` on a branch; Reject `init --force`; read `UPGRADE.md`. Optional `ui-polish` is after `frontend-design`, not instead; Design `setup` if `DESIGN.md` is missing; desktop and mobile. Optional `web-performance` is measure-first Web Vitals; not on default `init`. `product-copy` confirms Reader / Job / One action / Proof before drafting; `deslop` is last and includes a claim sweep. Do not import [coreyhaines31/marketingskills](https://github.com/coreyhaines31/marketingskills). Planner’s `planning` skill confirms Outcome / User / Why now / Success / Constraint / Out of scope, then names the owner and the session launches them with the USER_GUIDE spawn payload. Default specialists launch the next specialist in Handoff. Copilot and Antigravity continue in-thread with “now App engineer.” USER_GUIDE New feature is Plan → implement → Security if needed → Design if UI → Copy if public words → QA. USER_GUIDE.html first viewport is init plus say the change. Remaining kit-quality work is skip-intake in `ROADMAP.md`. Do not treat Phases 1–9 as the current queue.
...
Use `ROADMAP.md` as the source of truth for phased implementation status and next actions. Active work is **Phase 10** (playbook parity and default spawn-handoff). Phases 1–9 are historical. Wave 4 is shipped.
...
9. The workflow verifies public package installation with `node scripts/post-publish-verify.mjs`, which waits for `npm view`, installs the published package, runs `init --activate all`, then `doctor` and `adapter validate all`.
