...
Package-level research decisions are recorded in `DECISIONS.md`. Default pack skills live in `skills/<id>/SKILL.md`. `frontend-design` is a playbook with `setup` / `build` / `review` / `detect` modes; the 2026-09-12 GitHub scan is in `research/summaries/frontend-design-agent-skill-2026-09-12.md`. `accessibility-wcag` is a WCAG 2.1 AA playbook (keyboard in the running browser); scan notes are in `research/summaries/accessibility-wcag-skill-2026-09-12.md`. `testing-qa` is unit / regression / smoke with commands listed; it does not replace `browser-qa` or `supabase-auth-rls`. `ship` is go / no-go with named env, rollback, and `browser-qa` paths for UI. Optional `debug` (`agent-kit add skill debug`) is reproduce → localize → reduce → fix → guard; not on default `init`. Optional `docs` updates USER_GUIDE, CHANGELOG, and the living file that moved; Reject restoring the 17-doc OS. Optional `upgrade` is `agent-kit update` on a branch; Reject `init --force`; read `UPGRADE.md`. Optional `ui-polish` is after `frontend-design`, not instead; Design `setup` if `DESIGN.md` is missing; desktop and mobile. Planner’s `planning` skill prints a fenced USER_GUIDE paste for the owner and extra reviewers; it does not run the other agents. Default specialists print the next USER_GUIDE paste in Handoff after Done-when. Default skill YAML descriptions are unique and include Use-when trigger phrases. Generated Copilot instructions include one USER_GUIDE paste per default agent. USER_GUIDE New feature is Plan → implement → Security if needed → Design if UI → Copy if public words → QA. USER_GUIDE.html first viewport is init plus one Planner paste. Remaining kit-quality work is Phase 10 in `ROADMAP.md` (Wave 4 decision; do not start it). Do not treat Phases 1–9 as the current queue.
