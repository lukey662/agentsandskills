...
Launch the next specialist. Do not impersonate them. Use the USER_GUIDE prompt as the spawn payload. Reject printing a paste and stopping. Reject skipping QA screenshots because the chain is long.

If auth, data, or secrets changed, launch Security with:
...
If the UI changed, launch Design after Security (or instead if Security was not needed). Then launch QA. Always launch QA for behavior or UI changes:
