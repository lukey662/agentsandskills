...
One skill, mapped onto this stack. Do not install axe, Pa11y, an a11y MCP, or an accessibility agent roster. Visual proof stays in `browser-qa`. Access control stays in `supabase-auth-rls`: hiding a control is not an accessibility pass and not authorization.
...
| Bypass (2.4.1) | Skip link or landmarks so repeating chrome is skippable. |
...
- **Static HTML pages:** keep a skip link, a visible `:focus-visible` outline, and `lang` on `<html>`. Do not add `border-left` selection rails.
...
- Dropping a page's skip link or `:focus-visible` outline.
