...
- Accepting screenshots while unexpected console errors or failed same-origin requests go unnamed (`browser-qa`).

## Done when

You opened the running app, captured desktop and mobile screenshots, **read the images**, named unexpected console errors and failed same-origin requests (or `none`), ran applicable tests **and listed the commands**, and wrote accept / accept-with-nits / reject with image paths under `qa-evidence/`. For screens, `accessibility-wcag` ran: a keyboard-only pass on the changed flow, not a screenshot guess. A release also has a `ship` go or no-go with a named kill switch.
